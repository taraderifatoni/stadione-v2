  ...
  return (
    <div>
      ...
      <div style={{ padding: "0 16px 16px" }}>
        ...
        <Link href="/venues" style={{ display: "flex", alignItems: "center", gap: 8, background: "#fff15", borderRadius: 10, padding: "10px 14px", textDecoration: "none", cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 8, marginBottom: 20, overflowX: "auto", paddingBottom: 4 }}>
          {sportTypes.map((t, i) => (
            <Link key={t} href={`/venues?sport=${t.toLowerCase()}`} style={{ padding: "8px 16px", borderRadius: 20, background: i === 0 ? C.primary : C.surface, color: i === 0 ? "#fff" : C.textSec, fontSize: 12, fontWeight: 500, whiteSpace: "nowrap", border: i === 0 ? "none" : `1px solid ${C.border}`, cursor: "pointer", textDecoration: "none" }}>{t}</Link>
          ))}
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
          ...
          <Link href="/venues" style={{ fontSize: 12, color: C.primaryLight, textDecoration: "none" }}>Lihat semua</Link>
        </div>
